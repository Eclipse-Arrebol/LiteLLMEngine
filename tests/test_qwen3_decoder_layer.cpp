...
void check_close(float a, float b, float tol = 2e-4f) {
    if (std::fabs(a - b) > tol) {
...
void run_qwen3_decoder_layer_test(Device device) {
    const std::string device_name = device == Device::CPU ? "cpu" : "cuda";

    std::cout << "[test_qwen3_decoder_layer_" << device_name << "] start\n";
    ...
    std::vector<float> expected = decoder_layer_ref(
        hidden_states_cpu,
        position_ids_cpu,
        input_ln_weight,
        post_attn_ln_weight,
    ...
    Qwen3DecoderLayer layer(config);
    ...
    layer.load_weights(weights, "model.layers.0");

    if (!weights.empty()) {
        throw std::runtime_error("Qwen3DecoderLayer did not consume all weights");
    }

    if (!layer.initialized()) {
        throw std::runtime_error("Qwen3DecoderLayer should be initialized");
    }
    ...
    layer.forward(hidden_states, context, output);
    ...
    std::cout << "[test_qwen3_decoder_layer_" << device_name << "] passed\n";
}

void test_qwen3_decoder_layer_output_shape_mismatch() {
    std::cout << "[test_qwen3_decoder_layer_output_shape_mismatch] start\n";

    ModelConfig config = make_test_config();
    Qwen3DecoderLayer layer(config);

    std::vector<float> input_ln_weight = {1.0f, 1.0f, 1.0f};
    std::vector<float> post_attn_ln_weight = {1.0f, 1.0f, 1.0f};
    ...
    Tensor bad_output({1, 2}, DType::FP32, Device::CPU);
    ...
    try {
        layer.forward(hidden_states, context, bad_output);
    } catch (const std::runtime_error&) {
        caught = true;
    }

    if (!caught) {
        throw std::runtime_error("Expected Qwen3DecoderLayer output shape mismatch error");
    }

    std::cout << "[test_qwen3_decoder_layer_output_shape_mismatch] passed\n";
}
...
int main() {
    try {
        run_qwen3_decoder_layer_test(Device::CPU);
        run_qwen3_decoder_layer_test(Device::CUDA);

        test_qwen3_decoder_layer_output_shape_mismatch();
    } catch (const std::exception& e) {
        std::cerr << "[test_qwen3_decoder_layer] failed: " << e.what() << "\n";
        return 1;
    }

    std::cout << "[test_qwen3_decoder_layer] all passed\n";
    return 0;
}