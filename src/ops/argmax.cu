...
std::vector<int32_t> argmax_each_row_cpu(const Tensor& logits) {
    const float* ptr = logits.ptr<float>();

    const int64_t num_rows = logits.shape()[0];
    const int64_t vocab_size = logits.shape()[1];
    ...
    for (int64_t row = 0; row < num_rows; ++row) {
        const float* row_ptr = ptr + row * vocab_size;

        int32_t best_id = 0;
        float best_value = row_ptr[0];

        for (int64_t i = 1; i < vocab_size; ++i) {
            const float value = row_ptr[i];
        ...
        token_ids[static_cast<size_t>(row)] = best_id;
    }

    return token_ids;
}

__global__ void argmax_each_row_kernel(
    const float* logits,
    int64_t num_rows,
    int64_t vocab_size,
    ...
    if (tid == 0) {
        out_token_ids[row] = shared_indices[0];
    }
}

std::vector<int32_t> argmax_each_row_cuda(const Tensor& logits) {
    const int64_t num_rows = logits.shape()[0];
    const int64_t vocab_size = logits.shape()[1];

    constexpr int kBlockSize = 256;

    Tensor out({num_rows}, DType::INT32, Device::CUDA);
    out.zero_();
    ...
    out.copy_to_cpu(
        host_token_ids.data(),
        host_token_ids.size() * sizeof(int32_t)
    );

    return host_token_ids;
}
...
std::vector<int32_t> argmax_each_row(const Tensor& logits) {
    ...
    if (logits.device() == Device::CPU) {
        return argmax_each_row_cpu(logits);
    }

    if (logits.device() == Device::CUDA) {
        return argmax_each_row_cuda(logits);
    }

    throw std::runtime_error("Unsupported device for argmax_each_row");
}
